...
| `PG_SSL_REJECT_UNAUTHORIZED` | По умолчанию `false` для managed Postgres в serverless; включай `true`, только если chain полностью доверенная (в strict-режиме `sslmode` не переписывается) |
